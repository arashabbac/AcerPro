...
namespace AcerPro.AcceptanceTests.Customers;
...
public class UpdateCustomerSuccessfully : IClassFixture<TestingWebAppFactory<Program>>
{
    ...
    private readonly RegisterUserFormViewModel _customer = new()
    {
    ...
    private readonly RegisterUserFormViewModel _updatedCustomer = new()
    {
        Firstname = Faker.Name.First(),
        Lastname = Faker.Name.Last(),
        DateOfBirth = Faker.Identification.DateOfBirth(),
    ...
    {
        var response = await _httpClient.GetFromJsonAsync<APIResponseModel<UserDto>>($"/api/customers/{_customerId}");
